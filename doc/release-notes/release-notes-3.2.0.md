TARIAN Core version *3.2.0* is now available from:  <https://github.com/Tarian-Foundation/Tarian/releases>
...
A full report provided by TARIAN developers is available on the [TARIAN Website](https://tarian.org/fake-stake-official-tarian-report/), which includes additional findings, mitigation details, and resources for testing. This type of attack has no risk to users' privacy and does not affect their holdings.

### Wrapped Serials

On March 6th 2019, an attack was detected on the TARIAN network zerocoin protocol, or zTARN. The vulnerability allows an attacker to fake serials accepted by the network and thus to spend zerocoins that have never been minted. As severe as it is, it does not harm users’ privacy and does not affect their holdings directly.
...
Creation of these automint addresses is currently only available via the RPC/Console `createautomintaddress` command, which takes no additional arguments. The command returns a new TARIAN address each time, but addresses created by this command can be re-used if desired.
...
The RegTest network mode has been re-worked to once again allow for the generation of on-demand PoW and PoS blocks. Additionally, many of the existing functional test scripts have been adapted for use with TARIAN, and we now have a solid testing base for highly customizable tests to be written.
