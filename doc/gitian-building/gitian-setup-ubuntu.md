...
The build script we'll be using is contained in the TARN github repository ([contrib/gitian-build.py](https://github.com/Carbon-Reduction-Initiative/TARN/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the TARN repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/TARN-Project/TARN/master/contrib/gitian-build.py
...
- Clone the gitian-builder, gitian.sigs, tarian-detached-sigs, and tarian GitHub repos.
