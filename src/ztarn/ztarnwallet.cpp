...
CzTARNWallet::CzTARNWallet(CWallet* parent)
{
    this->wallet = parent;
    ...
    //Check for old db version of storing ztarn seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZTARNSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (wallet->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZTARNSeed_deprecated()) {
                    LogPrintf("%s: Updated zTARN seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old ztarn seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fRegtest && fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of ztarn wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzTARNWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZTARNCount(nCountLastUsed);
    else if (!walletdb.ReadZTARNCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzTARNWallet::Lock()
{
    seedMaster.SetNull();
}

void CzTARNWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzTARNWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZTARN(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzTARNWallet::LoadMintPoolFromDB()
{
...
void CzTARNWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzTARNWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzTARNWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (wallet->ztarnTracker->HasPubcoinHash(pMint.first)) {
...
bool CzTARNWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZTARN(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    wallet->ztarnTracker->Add(dMint, true);
...
void CzTARNWallet::SeedToZTARN(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
    libzerocoin::ZerocoinParams* params = Params().GetConsensus().Zerocoin_Params(false);
...
uint512 CzTARNWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzTARNWallet::GenerateDeterministicZTARN(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzTARNWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzTARNWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzTARNWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
