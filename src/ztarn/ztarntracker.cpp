...
#include "ztarn/ztarnwallet.h"


CzTARNTracker::CzTARNTracker(CWallet* parent)
{
    this->wallet = parent;
...
CzTARNTracker::~CzTARNTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzTARNTracker::Init()
{
...
bool CzTARNTracker::Archive(CMintMeta& meta)
{
...
bool CzTARNTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(wallet->strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzTARNTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzTARNTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzTARNTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzTARNTracker::GetSerialHashes()
{
...
CAmount CzTARNTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzTARNTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzTARNTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzTARNTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzTARNTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzTARNTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzTARNTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzTARNTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzTARNTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzTARNTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(wallet->strWalletFile);
...
void CzTARNTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzTARNWallet* zTARNWallet)
{
    bool iszTARNWalletInitialized = (nullptr != zTARNWallet);
    CMintMeta meta;
    ...
    if (!iszTARNWalletInitialized)
        zTARNWallet = new CzTARNWallet(wallet);
    meta.isSeedCorrect = zTARNWallet->CheckSeed(dMint);
    if (!iszTARNWalletInitialized)
        delete zTARNWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzTARNTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzTARNTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzTARNTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzTARNTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzTARNTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzTARNTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(wallet->strWalletFile);
    if (fUpdateStatus) {
        ...
        LogPrint(BCLog::LEGACYZC, "%s: added %d dztarn from DB\n", __func__, listDeterministicDB.size());
    }
