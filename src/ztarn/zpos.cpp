...
bool CLegacyZTarnStake::InitFromTxIn(const CTxIn& txin)
{
    // Construct the stakeinput object
    if (!txin.IsZerocoinSpend())
        return error("%s: unable to initialize CLegacyZTarnStake from non zc-spend", __func__);
    ...
    *this = CLegacyZTarnStake(spend);

    // Find the pindex with the accumulator checksum
    if (!GetIndexFrom())
        return error("%s : Failed to find the block index for ztarn stake origin", __func__);

    // All good
    return true;
}

CLegacyZTarnStake::CLegacyZTarnStake(const libzerocoin::CoinSpend& spend)
{
...
CBlockIndex* CLegacyZTarnStake::GetIndexFrom()
{
...
CAmount CLegacyZTarnStake::GetValue() const
{
    return denom * COIN;
}

CDataStream CLegacyZTarnStake::GetUniqueness() const
{
...
bool CLegacyZTarnStake::ContextCheck(int nHeight, uint32_t nTime)
{
    const Consensus::Params& consensus = Params().GetConsensus();
    if (!consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC_V2) || nHeight >= consensus.height_last_ZC_AccumCheckpoint)
        return error("%s : zTARN stake block: height %d outside range", __func__, nHeight);
