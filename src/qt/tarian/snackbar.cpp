...
#include "qt/tarian/forms/ui_snackbar.h"
#include "qt/tarian/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(TARIANGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &TARIANGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
