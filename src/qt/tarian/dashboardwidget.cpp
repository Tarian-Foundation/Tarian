...
#include "qt/tarian/dashboardwidget.h"
#include "qt/tarian/forms/ui_dashboardwidget.h"
#include "qt/tarian/sendconfirmdialog.h"
#include "qt/tarian/txrow.h"
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isTarn = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZTARN;
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair TARN, zTARN
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->append(chartData->valuesTarn);
    set1->append(chartData->valueszTarn);
    ...
    ui->labelAmountTarn->setText(GUIUtil::formatBalance(chartData->totalTarn, nDisplayUnit));
    ui->labelAmountZtarn->setText(GUIUtil::formatBalance(chartData->totalZtarn, nDisplayUnit, true));
