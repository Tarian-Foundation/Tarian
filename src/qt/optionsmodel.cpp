...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::TARN);
    ...
    if (settings.contains("nAnonymizeTarianAmount") || reset)
        SoftSetArg("-anonymizetarianamount", settings.value("nAnonymizeTarianAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in tarian.cpp during shotdown to also remove the window positions
