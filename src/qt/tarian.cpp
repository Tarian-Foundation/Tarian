...
public:
    ...
    /// Get window identifier of QMainWindow (TARNGUI)
...
{
    window = new TARNGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &TARNGUI::detectShutdown);
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &TARNGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(TARNGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(TARNGUI::DEFAULT_WALLET);
        ...
        // TARN: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &TARNGUI::handlePaymentRequest);
        connect(window, &TARNGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. TARN can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
