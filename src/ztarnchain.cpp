...
{
    CDataStream serializedCoinSpend = ZTARNModule::ScriptSigToSerializedSpend(txin.scriptSig);
...
bool UpdateZTARNSupplyConnect(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    //Add mints to zTARN supply (mints are forever disabled after last checkpoint)
    ...
    //Remove spends from zTARN supply
    ...
    // A one-time event where only the zTARN supply was off (due to serial duplication off-chain on main net)
...
bool UpdateZTARNSupplyDisconnect(const CBlock& block, CBlockIndex* pindex)
{
    ...
    // Re-add spends to zTARN supply
    std::list<libzerocoin::CoinDenomination> listDenomsSpent = ZerocoinSpendListFromBlock(block, true);
    for (const libzerocoin::CoinDenomination& denom : listDenomsSpent) {
        mapZerocoinSupply.at(denom)++;
    }

    // Remove mints from zTARN supply (mints are forever disabled after last checkpoint)
    if (pindex->nHeight < consensus.height_last_ZC_AccumCheckpoint) {
        std::list<CZerocoinMint> listMints;
        std::set<uint256> setAddedToWallet;
        BlockToZerocoinMintList(block, listMints, true);
