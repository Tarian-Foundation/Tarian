...
class ZerocoinSpendTest(TarnTestFramework):
    ...
    def run_test(self):
        ...
        ztarn_balance = self.nodes[2].getzerocoinbalance()['Total']
        assert_equal(balance, DecimalAmt(13833.92))
        assert_equal(ztarn_balance, 6666)
        ...
        ztarn_balance, balance = check_balances(denom_2, ztarn_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        ztarn_balance, balance = check_balances(denom_3, ztarn_balance, balance)
        ...
        _, _ = check_balances(denom_1, ztarn_balance, balance)
        self.log.info("--> VALID PUBLIC COIN SPEND (v3) PASSED")
